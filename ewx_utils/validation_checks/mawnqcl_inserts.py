...
logger = mawnqcl_logger()


def create_db_connections():
    """
    Create and return database connections and cursors for mawndb, mawndbqc, and rtma databases.
    """
...
def main():
    ...
    try:
        ...
        print(f"Number of Mawn_records: {len(mawn_records)}")
        rtma_records = fetch_records(rtma_cur, table_name, date)
        print(f"Number of rtma_records: {len(rtma_records)}")
    except Exception as e:
    ...
    try:
        insert_records(mawnqc_cur, "aetna_hourly", clean_records)
    except Exception as e:
