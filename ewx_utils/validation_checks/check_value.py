...
def check_value(k: str, v: float, d: dt.datetime):
    ...
    if k in relh_vars: 
        rh = humidity(v, 'PCT', d) #Obtain the value, the unit and the date as created in  the class humidity
    ...
    if k in pcpn_vars:
        pn = precipitation(v, 'hourly', 'MM', d) # Obtain the value, the table, the unit and the date
        if pn.IsValid():
            return True
        else:
            return False
    ...
    if k in rpet_vars:
        rp = evapotranspiration(v, 'hourly', 'MM', d) # Obtain the value, the table, the unit and the date
        if rp.IsValid():
            return True
        else:
            return False
    ...
    if k in temp_vars:
        tp = temperature(v, 'c', d) # Obtain the value, the unit and the date the value was retrieved
        if tp.IsValid():
            return True
        else:
            return False
    ...
    if k in wspd_vars:
        ws = windspeed(v, 'MPS', d) #Obtain the value, the unit and the date the value was retrieved
        if ws.IsValid():
            return True
        else:
            return False
    ...
    if k in wdir_vars:
        wd = winddirection(v, 'DEGREES', d) #Obtain the value, the unit and the date the value was retrieved
        if wd.IsValid():
            return True
        else:
            return False
    ...
    if k in leafwt_vars:
        lw = leafwetness(v, 'hourly', k, d) #Obtain the value, the table, the specific sensor ie lws01 or lws01 and the date 
