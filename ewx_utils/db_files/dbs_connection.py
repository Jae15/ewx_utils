...
def create_db_connections01(args: Namespace) -> Dict[str, Any]:
    """
    Create and return necessary database connections based on user-specified arguments.
    ...
    args : Namespace
        An object containing user-specified arguments for database connections.
        These could include arguments for multiple databases (e.g., 'mawn', 'rtma', 'qcwrite').
    ...
    OperationalError:
        If there is an operational error while connecting to the database.
    Exception
        For any unexpected errros that occur during the connection process.
    """
    my_dbfiles_logger.info("Creating necessary database connections based on user input.")
    connections = {}

    # Iterating over the user-specified databases provided in args
    for db_key, db_name in vars(args).items():
        if db_name:
            try:
                my_dbfiles_logger.info(f"Connecting to {db_key.upper()} database {db_name}.")
                db_config = get_db_config(db_name)
                connections[f"{db_key}_connection"] = connect_to_db(db_config)
            except OperationalError as e:
                my_dbfiles_logger.error(f"OperationalError: {e}")
                raise
            except Exception as e:
                my_dbfiles_logger.error(f"Unexpected error: {e}")
                raise
    my_dbfiles_logger.info("Database connections created successfully based on the required databases.")
    return connections
