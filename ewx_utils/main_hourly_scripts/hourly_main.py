...
def main() -> None:
    """
    Main function to check and update data from specified sections in the database.
    
    Accepts user-defined read and write database sections and processes records dynamically.
    """
    ...
    parser = argparse.ArgumentParser(
        prog="hourly_main",
        description="Checks data from specified sections and adds estimates from RTMA or fivemin data as needed",
    )
    parser.add_argument("-b", "--begin", type=str, help="Start date (YYYY-MM-DD, no time)")
    parser.add_argument("-e", "--end", type=str, help="End date (YYYY-MM-DD, no time)")

    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("-x", "--execute", action="store_true", help="Execute SQL and modify data")
    group.add_argument("-d", "--dryrun", action="store_true", help="Do not execute SQL, only output results")

    group = parser.add_mutually_exclusive_group()
    group.add_argument("-s", "--stations", nargs="*", type=str, help="Run for specific stations")
    group.add_argument("-a", "--all", action="store_true", default=False, help="Run for all stations")

    parser.add_argument("--read-from", type=str, nargs="+", required=True, help="Sections in INI file for reading")
    parser.add_argument("--write-to", type=str, required=True, help="Section in INI file for writing")
    ...
    # Establish database connections dynamically
    db_connections = create_db_connections(args)
    print(f"db_connections: {db_connections}")

    try:
        # Dynamically retrieve cursors for read connections
        read_cursors = {
            section: get_cursor(db_connections[f"{section}_connection"])
            for section in args.read_from
        }

        # Retrieve cursor for write connection
        write_cursor = get_cursor(db_connections[f"{args.write_to}_connection"])

        # Debug log cursors
        for section, cursor in read_cursors.items():
            my_logger.debug(f"Read cursor for {section}: {cursor}")

        my_logger.debug(f"Write cursor: {write_cursor}")

        # Determine stations
        if args.all:
            stations = get_all_stations_list(next(iter(read_cursors.values())))  # Use any read cursor
        else:
            stations = args.stations

        station_info = get_station_data(next(iter(read_cursors.values())))
        ...
        for station in stations:
            # Dynamically fetch records for all read sections
            read_records = {
                section: fetch_records(
                    cursor,
                    station,
            ...
            qc_columns = get_insert_table_columns(write_cursor, station)

            # Process records
            cleaned_records = process_records(qc_columns, read_records, begin_date, end_date)

            # If execution is enabled, insert or update records
            if args.execute:
                commit_and_rollback(db_connections[f"{args.write_to}_connection"], station, cleaned_records)
...
def main() -> None:
    ...
    # Create dedicated cursors for each database connection
    read_cursors = {}
    write_cursor = None

    try:
        # Open dedicated cursors for each read database
        for section in args.read_from:
            connection_key = f"{section}_connection"
            if connection_key in db_connections:
                read_cursors[section] = get_cursor(db_connections[connection_key])
            else:
                my_logger.error(f"No connection found for section: {section}")
                raise ValueError(f"No connection found for section: {section}")

        # Open a dedicated cursor for the write database
        write_key = f"{args.write_to}_connection"
        if write_key in db_connections:
            write_cursor = get_cursor(db_connections[write_key])
        else:
            my_logger.error(f"No connection found for write section: {args.write_to}")
            raise ValueError(f"No connection found for write section: {args.write_to}")
        ...
        # Fetch station metadata using the first available read cursor
        station_info = get_station_data(next(iter(read_cursors.values())))

        # Compute runtime date ranges
        runtime_begin_dates = get_runtime_begin_date(begin_date, station_info)
        runtime_end_dates = get_runtime_end_date(end_date, station_info)

        for station in stations:
            # Dynamically fetch records from all read databases
            read_records = {}
            for section, cursor in read_cursors.items():
                read_records[section] = fetch_records(
                    cursor,
                    station,
                    runtime_begin_dates[station],
                    runtime_end_dates[station],
            ...
            # Get table columns for the write database
            qc_columns = get_insert_table_columns(write_cursor, station)
            ...
            # If execution is enabled, insert or update records
            if args.execute:
                commit_and_rollback(db_connections[write_key], station, cleaned_records)
        ...
        for section, cursor in read_cursors.items():
            try:
                ...
                my_logger.warning(f"Error closing read cursor for {section}: {e}")

        if write_cursor:
            try:
                write_cursor.close()
            except Exception as e:
                my_logger.warning(f"Error closing write cursor: {e}")
